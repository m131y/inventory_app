...
public class StockQueryRepository {
    ...
    public List<Stock> search(StockSearchCondition condition){
        ...
        // 최종 쿼리 실행
        // selectFrom(stock): Stock 기준 조회
        // join(...): 실제 연관된 Product/Warehouse를 조인 (inner join)
        // where(builder): 위에서 누적된 조건 적용
        // orderBy(...): 결과 정렬 (상품명, 창고명 오름차순)
        // fetch(): 최종 결과 리스트 반환
        return queryFactory
                .selectFrom(stock)
